...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float xDisplacementCount = (sin(timevalue) / 2.0f);
		int vertexdisplacementLocation = glGetUniformLocation(shaderProgram, "Displacement");
		glUseProgram(shaderProgram);
		glUniform3f(vertexdisplacementLocation, xDisplacementCount, 0.0, 0.0);
